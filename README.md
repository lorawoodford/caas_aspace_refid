...
The second migration `migrations/002_seed_initial_values.rb` will do the work of seeding the initial data required by the plugin.  To prepare for this migration, you will need a csv in the following format.  For the initial migrations on test and prod, these csv's were generated by the queries found in `migrations/csvs/initial_seed_query.sql`.
